...
def render_motion(project_dir: Path, motion_renderer: str | None = None, *, draft_ok: bool = False) -> tuple[Path, Path, list[dict[str, str]]]:
    ...
    required_motions = index_payload.get("required_motions") if isinstance(index_payload, dict) else None
    if required_motions is None:
        required_motions = []
        failures = [failure("required_motion_index_missing", "S5 must consume work/plan/required_motion_index.json generated by S2.")]
    else:
        failures = []
    ...
    if assertions_payload.get("generated_by") != "short_video_engine":
        failures.append(failure("motion_assertions_missing", "S5 must consume motion_assertions.json produced by S2."))
    if icon_manifest.get("generated_by") != "short_video_engine":
        failures.append(failure("motion_icon_manifest_missing", "S5 must consume motion_icon_manifest.json produced by S4.5."))
...
def segment_for_required_motion(required_motion: dict[str, Any], shots_by_id: dict[str, dict[str, Any]], all_shots: list[dict[str, Any]], cues_by_id: dict[str, dict[str, Any]], index: int, assertion: dict[str, Any] | None, icon_manifest: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    segment = build_logic_segment_group(group or [shot], cues_by_id, index) if shot else {
        "logic_segment_id": f"logic_{index:03d}_{shot_id or 'missing'}",
        "shot_id": shot_id,
        "covered_shot_ids": [shot_id] if shot_id else [],
        "subtitle_cue_ids": required_motion.get("cue_ids") or [],
        "required_intervals": [{"start": required_motion.get("start_sec"), "end": required_motion.get("end_sec")}],
        "actual_intervals": [{"start": required_motion.get("start_sec"), "end": required_motion.get("end_sec")}],
        "logic_relation": "",
        "logic_entities": [],
        "visual_claim": short_label(str(required_motion.get("source_text") or ""), "逻辑"),
        "motion_text_items": [],
    }
    segment["motion_id"] = str(required_motion.get("motion_id") or "")
    if assertion:
        ...
        segment.update(
            {
                ...
                **semantic_relation_fields(assertion),
            }
        )
    return segment
...
def semantic_relation_fields(assertion: dict[str, Any]) -> dict[str, Any]:
...
def used_icon_ids(segment: dict[str, Any]) -> list[str]:
    icons = segment.get("semantic_icons") if isinstance(segment.get("semantic_icons"), dict) else {}
    return sorted({str(value.get("icon_id") or "") for value in icons.values() if isinstance(value, dict) and value.get("icon_id")})


def interval_duration(segment: dict[str, Any]) -> float:
    intervals = segment.get("required_intervals") if isinstance(segment.get("required_intervals"), list) else []
    if not intervals or not isinstance(intervals[0], dict):
        return 2.4
    try:
        return max(0.0, float(intervals[0].get("end") or 0) - float(intervals[0].get("start") or 0))
    except (TypeError, ValueError):
        return 2.4
...
def render_segment(project_dir: Path, segment: dict[str, Any], scene: dict[str, Any] | None = None, requested_renderer: str = "auto", *, draft_ok: bool = False, remotion_decision: dict[str, Any] | None = None) -> tuple[dict[str, Any], list[dict[str, str]]]:
    ...
    frame_count = max(1, round(max(0.8, interval_duration(segment)) * 30))
    if use_remotion:
        ...
        if not render_failures:
            ...
            layer = {
                ...
                **artifact_info,
                ...
                "icon_manifest_path": str(plan_dir(project_dir) / "motion_icon_manifest.json"),
                "icon_manifest_hash": hashlib.sha256((plan_dir(project_dir) / "motion_icon_manifest.json").read_bytes()).hexdigest(),
                "used_icon_ids": used_icon_ids(segment),
        ...
        if not render_failures:
            ...
            layer = {
                ...
                **artifact_info,
                ...
                "professional_quality_status": "passed",
                "semantic_readability_status": "passed",
                "template_stage_count": max(len(segment.get("required_visual_actions") or []), len(stage_contract_for(template, relation)), 5),
                "overlay_compositing_mode": "transparent_rgba_overlay",
                "alpha_channel_status": "passed",
                "background_alpha_policy": "transparent_background_required",
                "required_animation_stages_completed": True,
    ...
    layer = {
        **semantic_layer_fields(segment),
        ...
        "logic_relation_reason": segment.get("logic_relation_reason") or {},
        "logic_entities": segment.get("logic_entities") or [],
        "animation_stages": segment.get("required_visual_actions") or ["start", "build", "mid", "emphasis", "end"],
        "visual_claim": segment.get("visual_claim"),
        "expression_plan": expression_plan,
        "motion_text_items": segment.get("motion_text_items") or [],
        "motion_pattern_family": template,
...
def semantic_layer_fields(segment: dict[str, Any]) -> dict[str, Any]:
    required_actions = [str(item) for item in (segment.get("required_visual_actions") or []) if str(item).strip()]
...
def semantic_visual_proof_for(action: str) -> dict[str, bool]:
    proof = {"non_decorative_scene": True, "has_relationship_flow": True}
    if action == "negate_and_redefine":
        proof.update({"has_rejection": True, "has_connection_flow": True})
    elif action == "connector_metaphor":
        proof.update({"has_connection_flow": True})
    elif action == "metric_growth":
        proof.update({"has_metric_delta": True})
    elif action == "process_migration":
        proof.update({"has_transition": True})
    elif action == "density_comparison":
        proof.update({"has_pressure": True, "has_expansion": True, "has_comparison_axis": True})
    elif action == "before_after_change":
        proof.update({"has_transition": True, "has_comparison_axis": True})
