...
export const NodeRelationTriangle: React.FC<MotionLayerProps> = (props) => {
  ...
  return (
    <Panel accent="#65e7ff">
      <div style={{position: 'absolute', left: 192, top: 178, opacity: a}}><IconBlock accent="#6eefff" label={props.input || '原因'} /></div>
      <div style={{position: 'absolute', left: 612, top: 178, opacity: b}}><IconBlock accent="#72ebcb" label={props.connector || '机制'} /></div>
      <div style={{position: 'absolute', left: 402, top: 410, opacity: c}}><IconBlock accent="#ffd84f" label={props.output || props.result || '结果'} /></div>
