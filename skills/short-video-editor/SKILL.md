...
HyperFrame required trigger: for any shot expressing 2+ KPI changes, cost/efficiency/risk pressure, process migration, before/after comparison, or `not X but Y` decision logic, default to `renderer: hyperframe` or `ae_overlay`. Downgrade only after writing `downgrade_reason` and a user-visible warning.
...
- Required-trigger HyperFrame/AE shots may be downgraded only with `downgrade_reason`, `why_simple_broll_is_enough`, and a user-visible warning in the plan.
...
- Visual QA must fail layout QC if preview frames show subtitles clipped, cropped by player controls, too long horizontally, or occupying more than two balanced lines; revise cue splitting before render.
